...
#define GPIO_TOUCH_IRQ 		MSM_GPIO_TO_INT(61)
...
//	void resumeWork (void);
	void resumeWork (struct work_struct *work);
#endif
...
u32 BUTTON_MAPPING_KEY[SUPPORTED_BUTTON_NUM] = {KEY_BACK, KEY_HOMEPAGE, KEY_MENU};
...
void resumeTimerCallback (unsigned long dummy) {
...
static void sain_late_resume(struct early_suspend *h)
{
	...
	enable_irq(touch_dev->client->irq);	 //ST_LIM
#ifdef POWER_OFF_SUSPEND
#else
...
/*
//static int sain_resume(struct i2c_client *client, pm_message_t message) {
static int sain_resume(struct i2c_client *client) {
...
static void sain_early_suspend(struct early_suspend *h)
{
	...
	//cancel_work_sync(&touch_dev->work);
#endif
	disable_irq(touch_dev->client->irq);	 //ST_LIM
#ifdef POWER_OFF_SUSPEND
#else	
#if	SAIN_ESD_TIMER_INTERVAL	
	if(touch_dev->use_esd_timer)
...
static int sain_touch_probe(struct i2c_client *client, const struct i2c_device_id *i2c_id)
{
	...
	set_bit(KEY_HOMEPAGE, touch_dev->input_dev->keybit);
	...
	if (touch_dev->client->irq) {
		ret = request_irq(touch_dev->client->irq, ts_int_handler, IRQF_TRIGGER_FALLING/*IRQF_TRIGGER_LOW|IRQF_DISABLED*/, SAIN_DRIVER_NAME, touch_dev);
		if (ret) {
