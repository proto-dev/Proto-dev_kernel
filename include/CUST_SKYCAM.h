 ...
 * 간단하게 인터페이스를 구현한다. * QC FD 적용 후에도 (만약 YUV 에도 사용 가능하다면...) 관련 코드 수정은 
 * 필요하므로 별도의 feature 로 분리한다. */
#define F_PANTECH_CAMERA_FD

#define F_SKYCAM_FIX_PREVIEWSIZE
...
#ifdef F_SKYCAM_QBUG_CTS
#define F_SKYCAM_VIDEO_TABLE /*20120510.WSH, Camera CTS문제로 Preview size table, Video size table분리 */
#define F_SKYCAM_LIVESNAPSHOT_PICTURESIZE  /*20120521.WSH, Camera testVideoSnapshot CTS 항목 Qualcomm SR내용 적용.  */
#endif
...
/* 20120303. WSH, EF34K ICS Camera snapshot pictures Rotation되는 현상 방지. */ 
#define F_SKYCAM_JPEG_ENCODE_ROTATION


/* 20120309. WSH, EF34K ICS video recording시 640X480, 320X240해상도 에서 encoding깨지는 현상 수정.  */ 
#define F_SKYCAM_RECORD_ENCODE


/* 20120405. WSH, EF34K ICS video recording시 live takeshot기능 적용.  */ 
#define F_SKYCAM_LIVE_TAKESHOT


/* 20120411. WSH, EF34K ICS 140patch시 camera parameter값 변경으로 VT call시 전면 카메라 보이지 않는 문제 수정.  */ 
#define F_SKYCAM_VT_ICS_PARAMETER


/* 20120413. WSH, EF34K ICS 140patch후 hal단 변경으로 인해 1920X1080해상도 지원code 추가 수정이 필요하여 수정함.  */ 
#ifdef F_SKYCAM_1080P_PREVIEW
#define F_SKYCAM_ICS_1080P_PREVIEW
#endif
...
#define F_SKYCAM_KERNEL_LOG_BUILD_MODE
#endif
